...
function enclose(content, framewidth, frameheight, contentX, contentY) {
	...
	content.style.right = contentY + "px";
	...
	function wheelHandler(event) {
		...
		var deltaY = (
			...
			e.deltaY* -30 ||
			e.wheelDeltaY /4 ||
			(e.wheelDeltaY === undefined && e.wheeDelta /4) ||
			e.detail* -10 ||
			0
		);
		if(isMacWebkit) {
		...
		if(e.altKey) {
			...
			if(deltaX) {
				...
				contentX = Math.max(contentX + deltaX, Math.min(framewidth - contentwidth, 0));
				contentX = Math.min(contentX, 0);
				content.style.left = contentX + "px";
			}
			if(deltaY) {

				contentY = Math.max(contentY + deltaY, Math.min(frameheight - contentheight, 0))
