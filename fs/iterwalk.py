...
def fstree(root :str | Path) -> Generator[Tuple[Path, List[Path], List[Path]] | List, None,None] :
	...
	"""
		...
		Not restricted by recursion limit and uses breadth-first algorithm, so must safe some memory.
		Also "deque" as a maintainable data structure must safe some time.
		...
		list of Path objects by invokation of it's (perhaps absent in this case) "iterdir" Path object method,
